...
LatestRoom.Changed:Connect(function()
    if not opened and LatestRoom.Value == 1 then
        ...
        task.spawn(function()
            ...
            while true do
                SyncWait(c + 80)
                if LatestRoom.Value >= 1 then LatestRoom.Changed:Wait() LoadEntity("Ripper") end
                SyncWait(c + 167)
                if LatestRoom.Value >= 1 then LatestRoom.Changed:Wait() LoadEntity("Ripper") end
                c = c + 300
                task.wait(1)
        ...
        -- FROSTBITE (Super Raro)
        task.spawn(function()
            local c = 0
            while true do
                SyncWait(c + 415)
                if LatestRoom.Value >= 1 then LatestRoom.Changed:Wait() LoadEntity("Frostbite") end
                SyncWait(c + 810)
                if LatestRoom.Value >= 1 then LatestRoom.Changed:Wait() LoadEntity("Frostbite") end
                c = c + 850
                task.wait(1)
        ...
        -- REBOUND (Ajustado: Mais frequente que Frostbite)
        task.spawn(function()
            local c = 0
            while true do
                SyncWait(c + 120)
                if LatestRoom.Value >= 1 then LatestRoom.Changed:Wait() LoadEntity("Rebound") end
                SyncWait(c + 280)
                if LatestRoom.Value >= 1 then LatestRoom.Changed:Wait() LoadEntity("Rebound") end
                c = c + 400
                task.wait(1)
